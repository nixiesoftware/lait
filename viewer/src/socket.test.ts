...
function frame(laneId: number, body: number[], version = socketProtocolVersion): Uint8Array {
...
describe("socket protocol frames", () => {
  ...
  it("refuses a frame past the ceiling before reading it", () => {
    ...
    const oversize = new Uint8Array(maxSocketFrameBytes + 1);
  ...
  it("refuses a frame from another build", () => {
    ...
    expect(decodeFrame(frame(lane.progress, body, socketProtocolVersion + 1))).toBeNull();
  });
