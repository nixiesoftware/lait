...
use crate::body::{BodyId, BodyKey, WorldId};
use crate::frontier::AuthorityFrontier as AF;
use crate::frontier::{AuthorityFrontier, ReplicaFrontier};
use crate::index::{node_hash, ChildRef, IndexNode, NodeSink, NodeSource};
use crate::manifest::{
    build_body_index, Invalid, ManifestBook, ManifestEntry, ManifestHead, ManifestRoot,
    RootObservation,
};
use crate::transaction::{AuthoritySource, SeedSigner};
...
impl AuthoritySource for BothSigners {
    fn signer_authorized(&self, signer: &[u8; 32], _f: &AF) -> bool {
        let s1 = mechanics::actor::device_from_seed(&SIGNER_SEED)
            .key_bytes()
            .unwrap();
        let s2 = mechanics::actor::device_from_seed(&OTHER_SEED)
            .key_bytes()
            .unwrap();
        *signer == s1 || *signer == s2
    }
...
fn a_substituted_entry_is_caught_by_its_placement() {
    ...
    let swapped = decoded.encode().expect("encode tampered index");
