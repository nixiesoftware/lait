...
use crate::cache::{Failure, Lease, Residency};
...
const fn operation(n: u8) -> [u8; 16] {
    [n; 16]
}

#[test]
fn an_installed_entry_reads_back_with_its_sidecar() {
    let cache = Residency::open(temp_root("install"), 1 << 20).unwrap();
...
fn a_slot_returns_exactly_what_was_put_in_it() {
    ...
    let cache = Residency::open(&root, 1 << 20).unwrap();
    let slot = [7u8; 32];
...
fn an_absent_entry_is_not_resident_rather_than_an_integrity_failure() {
    ...
    let cache = Residency::open(temp_root("absent"), 1 << 20).unwrap();
...
fn an_interrupted_install_is_reclaimed_on_open() {
    ...
    let _cache = Residency::open(&root, 1 << 20).unwrap();
...
fn two_operations_holding_one_entry_both_have_to_release_it() {
    ...
    let cache = Residency::open(temp_root("leases"), 0).unwrap();
...
fn taking_a_lease_twice_holds_it_once() {
    let cache = Residency::open(temp_root("idempotent"), 0).unwrap();
...
fn a_crashed_operation_releases_every_lease_it_held() {
    ...
    let cache = Residency::open(temp_root("crashed"), 0).unwrap();
...
fn a_pin_survives_quota_pressure() {
    let cache = Residency::open(temp_root("pin"), 0).unwrap();
...
fn a_sweep_inside_quota_evicts_nothing() {
    ...
    let cache = Residency::open(temp_root("under"), 1 << 30).unwrap();
...
fn staged_bytes_are_never_advertised_and_resume_from_where_they_are() {
    let cache = Residency::open(temp_root("staging"), 1 << 20).unwrap();
    let op = operation(3);
...
fn staging_for_a_dead_operation_is_swept() {
    let cache = Residency::open(temp_root("deadstage"), 1 << 20).unwrap();
...
fn a_range_read_returns_only_what_was_asked_for() {
    let cache = Residency::open(temp_root("range"), 1 << 20).unwrap();
...
fn eviction_reclaims_the_most_room_for_the_fewest_refetches() {
    ...
    let cache = Residency::open(temp_root("largest"), 128).unwrap();
    let big = vec![1u8; 400];
...
fn an_operation_release_cannot_drop_a_content_hold() {
    ...
    let cache = Residency::open(temp_root("kinds"), 1 << 20).unwrap();
    let bytes = b"ciphertext".to_vec();
    let entry = address(&bytes);
    cache.install(&entry, &bytes, b"proof").unwrap();
...
fn a_sweep_that_cannot_reach_the_quota_says_so() {
    ...
    // something.
    let cache = Residency::open(temp_root("shortfall"), 16).unwrap();
...
fn an_unreadable_tag_directory_stops_the_sweep_rather_than_freeing_everything() {
    ...
    let cache = Residency::open(&root, 0).unwrap();
    let bytes = b"ciphertext".to_vec();
    let entry = address(&bytes);
    cache.install(&entry, &bytes, b"proof").unwrap();
...
fn finishing_one_staged_part_leaves_the_others_alone() {
    ...
    let cache = Residency::open(temp_root("staged-part"), 1 << 20).unwrap();
    let op = operation(3);
    for part in 0..4u32 {
        cache
...
fn a_dead_operations_lease_is_released_and_a_content_hold_is_not() {
    ...
    let cache = Residency::open(temp_root("sweep-leases"), 1 << 20).unwrap();
    let bytes = b"ciphertext".to_vec();
    let entry = address(&bytes);
    cache.install(&entry, &bytes, b"proof").unwrap();
