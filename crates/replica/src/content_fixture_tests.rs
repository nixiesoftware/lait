...
fn a_chunk_cannot_be_moved_lifted_or_regeometried() {
    ...
    let moved = ChunkProof {
        leaf: ChunkLeaf::of(0, &ciphertexts[1]).expect("bounded leaf"),
    ...
    assert!(
        mechanics::authorization::content_chunk_open(&key(), &wrong_index_binding, &ciphertexts[1])
            .is_none(),
    ...
    let other = seal_content(&space(), &key(), [6u8; 16], &plaintext)
        ...
        .descriptor;
    assert!(
        mechanics::authorization::content_chunk_open(&key(), &other.binding(1), &ciphertexts[1])
            .is_none(),
...
fn the_tree_promotes_rather_than_duplicating_an_odd_node() {
    ...
    let leaves: Vec<ChunkLeaf> = (0..3)
        .map(|i| ChunkLeaf::of(i, &filler(i as u64, 64)).expect("bounded leaf"))
        .collect();
...
fn every_chunk_proves_against_the_root() {
    for count in [1usize, 2, 3, 4, 5, 8, 9, 17, 64] {
        let leaves: Vec<ChunkLeaf> = (0..count)
            .map(|i| ChunkLeaf::of(i as u32, &filler(i as u64, 128)).expect("bounded leaf"))
            .collect();
...
fn recorded_chunk_geometry() {
    ...
    assert!(
        crate::content::max_ciphertext_len() < 1024 * 1024,
...
fn a_cancelled_ingest_leaves_nothing_reachable() {
    use crate::content::ContentIngest;
    let cache = temp_cache("cancel");
    let mut ingest =
        ContentIngest::begin(&space(), &key(), [2u8; 16], &cache, u64::MAX).expect("begin ingest");
    ingest
...
fn an_ingest_past_its_policy_maximum_is_refused_while_streaming() {
    ...
    let mut ingest =
        ContentIngest::begin(&space(), &key(), [4u8; 16], &cache, 1_000).expect("begin ingest");
...
fn ingest_holds_one_chunk_regardless_of_content_size() {
    ...
    let mut ingest =
        ContentIngest::begin(&space(), &key(), [5u8; 16], &cache, u64::MAX).expect("begin ingest");
...
fn the_ingest_hold_and_the_content_hold_hand_over() {
    ...
    let cache = crate::cache::Residency::open(cache.root(), 0).unwrap();
    let mut ingest =
        ContentIngest::begin(&space(), &key(), [7u8; 16], &cache, u64::MAX).expect("begin ingest");
