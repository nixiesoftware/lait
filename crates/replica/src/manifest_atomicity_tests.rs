...
use crate::transaction::{ActionOutcome, CommitAuthorization, CommitContext, SeedSigner};
use crate::Replica;
...
) -> Result<crate::convergence::ConvergenceOutcome, crate::transaction::commit::Failure> {
    let (space, signer) = ctx_parts();
    let ctx = CommitContext {
        space: &space,
        signer: &signer,
...
fn a_second_transaction_failure_commits_nothing() {
    ...
    assert_eq!(err, crate::transaction::commit::Failure::QuotaExceeded);
...
fn kill_and_reopen_at_every_journal_boundary_shows_old_or_complete_new_root() {
    for point in journal::FAULT_POINTS {
        ...
        let dir = temp_store("fault");
        {
            ...
            let mut b = Replica::open(&dir, keys())
                .unwrap()
