...
fn a_non_canonical_shape_is_refused() {
    ...
    let a_bytes = leaf_a.encode().expect("encode leaf");
    let b_bytes = leaf_b.encode().expect("encode leaf");
    ...
    let branch_bytes = branch.encode().expect("encode branch");
    sink.written.push(a_bytes);
    sink.written.push(b_bytes);
...
const fn journal_test_nibble(key: &IndexKey) -> usize {
    (key[0] >> 4) as usize
}
...
fn a_count_that_lies_is_refused() {
    ...
    let lying_bytes = lying.encode().expect("encode branch");
    let mut sink = NodeSink::default();
