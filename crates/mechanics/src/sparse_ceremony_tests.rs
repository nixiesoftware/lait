...
fn ten_thousand_ordinary_operations_invoke_no_ceremony() {
    ...
    let mut ledger = Authority::create(&dir, genesis.clone()).unwrap();
    let (member_incept, member) = actor::incept_single(&seed(2), &space, [2; 16], [72; 16], None);
    ledger
        .commit_batch(
            &[
                Effect::Actor(incept).encode(),
                Effect::Actor(member_incept).encode(),
            ],
            &[],
        )
        .unwrap();
    ...
    let member_key = mechanics::actor::device_from_seed(&seed(2))
        .key_bytes()
        .unwrap();
    let stranger_key = mechanics::actor::device_from_seed(&seed(9))
        .key_bytes()
        .unwrap();
...
fn an_explicit_ceremony_command_is_the_positive_control() {
    ...
    let mut ledger = Authority::create(&dir, genesis).unwrap();
    ledger
        .commit_batch(&[Effect::Actor(incept).encode()], &[])
        .unwrap();
    ...
    let proposal = mechanics::dkg::sign_ceremony(
        ...
        &mechanics::dkg::CeremonyOp::DkgPropose(mechanics::dkg::frost_rotation_proposal(
            [7u8; 16],
            1,
            vec![mechanics::authority::Principal::of_device(
                &mechanics::actor::device_from_seed(&seed(1)),
            )],
            mechanics::authority::Authority::single(mechanics::space::recovery_pub_of(&seed(1))),
        )),
        &space,
    );
    ledger
        .commit_ceremony_batch(&[mechanics::recovery::CeremonyMaterial::new(proposal).encode()])
        .unwrap();
    ...
    let terminal = mechanics::space::sign_op(
        ...
        &mechanics::space::SpaceOp::Rotate {
            ...
            next_configuration: mechanics::authority::ConfigId::single(),
            gen: 1,
        },
        vec![],
        &space,
    ...
    ledger
        .commit_batch(&[Effect::SpaceAuthority(terminal).encode()], &[])
        .unwrap();
    assert_eq!(
