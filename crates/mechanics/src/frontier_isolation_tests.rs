...
fn signing_transcript(fx: &Fx, nonce_tag: u32, op_bytes: Vec<u8>) -> Vec<SignedSpaceEvent> {
    ...
    let coordinator = actor::device_from_seed(&seed(1));
...
fn ceremony_traffic_never_enlarges_the_ordinary_frontier() {
    ...
    let fx = fx();
    let mut ledger = SpaceAuthority::create(&dir, fx.genesis.clone()).unwrap();
    ledger
    ...
    let err = ledger
        .commit_batch(&[Effect::SpaceAuthority(ceremony_node).encode()], &[])
        .unwrap_err();
    assert!(
        matches!(err, mechanics::space::Failure::InvalidRecord),
    ...
    ledger
        .commit_batch(&[Effect::SpaceAuthority(terminal_node).encode()], &[])
        .unwrap();
    ...
    drop(ledger);
    let mut ledger = SpaceAuthority::open(&dir).unwrap();
    assert_eq!(ledger.ceremony_cursor(), total);
    ...
    let mut peer = SpaceAuthority::create(&dir_b, fx.genesis.clone()).unwrap();
    ...
    assert!(matches!(err, mechanics::space::Failure::InvalidRecord));

    let _ = (
        Authority::single(space::recovery_pub_of(&seed(40))),
        Principal::of_device(&actor::device_from_seed(&seed(1))),
    );
