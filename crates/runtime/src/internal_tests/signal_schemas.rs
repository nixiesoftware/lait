...
mod declarations {
    ...
    fn build(scopes: Vec<ScopeSchema>, signals: Vec<SignalSchema>) -> Result<(), Refusal> {
        ...
        Builder::new().register(Arc::new(world)).build().map(|_| ())
    }
...
mod delivery {
    ...
    async fn a_signal_with_nobody_listening_is_not_a_failure() {
        ...
        handle.deliver(runtime::signal::DeliveredSignal {
            from: mechanics::station::Key::from_device(&mechanics::actor::device_from_seed(
                &[5u8; 32],
            ))
            .expect("station"),
    ...
    fn from(seed: u8) -> mechanics::station::Key {
        mechanics::station::Key::from_device(&mechanics::actor::device_from_seed(&[seed; 32]))
            .expect("station")
    }
