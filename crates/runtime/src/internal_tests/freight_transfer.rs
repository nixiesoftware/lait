...
fn two_operations_over_one_content_do_not_collect_each_others_staged_bytes() {
    ...
    let entry = journal::object_content_hash(b"chunk-0");
    ...
    for op in [first.operation(), second.operation()] {
        ...
        cache.hold_operation(op, entry).unwrap();
    }
...
fn a_dropped_transfer_fails_itself_and_lets_go() {
    ...
    let registry = Arc::new(TransferRegistry::new());
    let now = Instant::now();
    let entry = journal::object_content_hash(b"orphan");
...

    {
        ...
        cache
            ...
            .unwrap();
        cache.hold_operation(handle.operation(), entry).unwrap();
        handle.advance(
