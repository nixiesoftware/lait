...
async fn wire(tag: &str, plaintext: Vec<u8>) -> Wire {
    ...
    let signer = replica::SeedSigner(&PROVIDER_SEED);
    ...
    let provider_transport: Arc<dyn comms::Transport> =
        Arc::new(net.peer(mechanics::crypto::device_from_seed(&PROVIDER_SEED)));
    let client_transport: Arc<dyn comms::Transport> =
        Arc::new(net.peer(mechanics::crypto::device_from_seed(&CLIENT_SEED)));
