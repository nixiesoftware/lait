//! The word list an address is minted from.
//!
//! This is the BIP-39 English word list, reproduced verbatim. It is used as a
//! *word list* and nothing else: no address here is a key, a seed, a secret or a
//! recovery phrase, and none of BIP-39's derivation is present in this tree.
//!
//! It was taken rather than improvised because the properties an address needs
//! are exactly the ones it was curated for and that a hand-picked list would
//! only claim: 2048 entries, lowercase ASCII, three to eight letters, unique in
//! the first four characters, and screened for homophones and confusable pairs.
//! `words::tests` asserts every one of those rather than trusting the
//! provenance.
//!
//! # Attribution
//!
//! From BIP-39 (Palatinus, Rusnak, Voisine, Bowe), which is published under the
//! 2-clause BSD licence. That licence covers this file's contents; the crate's
//! own terms are unchanged. Source:
//! <https://github.com/bitcoin/bips/blob/master/bip-0039/english.txt>
//!
//! # Why the list is not consulted when parsing
//!
//! [`super::address::Address::parse`] checks *shape* only. Membership is a
//! minting-time question: an address issued under this list has to keep parsing
//! if the list is ever revised, and a parser that rejected a word retired from a
//! later edition would break addresses already spoken aloud and written down.
//! So issuance draws from here, and parsing does not.

/// The words, in the list's own order. Index is the value; the order is fixed
/// because an address's words are drawn by index.
pub static WORDS: [&str; super::address::WORD_COUNT] = [
...
mod tests {
    ...
    /// Every property the address format leans on, asserted here rather than
    /// inherited from where the list came from.
    #[test]
    fn the_list_has_the_shape_an_address_is_minted_from() {
        assert_eq!(WORDS.len(), super::super::address::WORD_COUNT);
        for word in WORDS {
            assert!(
                (3..=8).contains(&word.len()),
                "{word} is not three to eight letters"
            );
            assert!(
    ...
    /// Two entries that agreed in their first four characters would make two
    /// addresses that a person reading one aloud could not keep apart.
    #[test]
    fn no_two_words_agree_in_their_first_four_letters() {
        ...
        assert_eq!(
            before,
            prefixes.len(),
            "two words share a four-letter prefix"
        );
    }

    /// A duplicate would silently shrink the keyspace the no-enumeration
    /// position is measured against.
    #[test]
    fn every_word_appears_once_and_the_list_is_ordered() {
        let mut sorted = WORDS.to_vec();
        sorted.sort_unstable();
        let ordered = sorted.clone();
        sorted.dedup();
        assert_eq!(sorted.len(), WORDS.len(), "the list repeats a word");
        assert_eq!(
            ordered.as_slice(),
            WORDS.as_slice(),
