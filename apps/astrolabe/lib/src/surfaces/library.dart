...
/// Author an MCP binding for this World. The editor parents `lait mcp`;
/// this client writes the pin (`LAIT_WORLD`) and never holds the process.
class _AgentBinding extends StatefulWidget {
  const _AgentBinding({required this.showing});

  final LibraryRow showing;

  @override
  State<_AgentBinding> createState() => _AgentBindingState();
}

class _AgentBindingState extends State<_AgentBinding> {
  String _client = 'claude';
  String _project = '';
  ...
  Widget build(BuildContext context) {
    ...
    final client = ClientScope.of(context);
    final view = ClientScope.watch(context);
    final project = _project.trim().isEmpty ? null : _project.trim();
    final mount = widget.showing.worldMount;
    final name = 'lait-$mount';
    final writing = view.inFlight.contains('mcp.install');
    final previewing = view.inFlight.contains('mcp.preview');
    final ready = project != null && !writing && !previewing;
    final outcome =
        view.mcp != null && view.mcp!.world == mount ? view.mcp : null;

    return Container(
      key: const ValueKey('library-agent-binding'),
      padding: t.padding.symmetric(h: Space.xl5, v: Space.xl3),
      decoration: BoxDecoration(
        border: t.stroke.edge(bottom: context.border.l500),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('AGENT BINDING', style: context.factLabelStyle),
          t.gap.y(Space.xs),
          Text(
            'Writes the binding that points an editor at this World. '
            'The editor owns that process; this client never stops it.',
            style: context.proseStyle,
          ),
          t.gap.y(Space.xl),
          Wrap(
            spacing: t.size.md,
            runSpacing: t.size.sm,
            children: [
              for (final choice in const [
                ('claude', 'Claude Code'),
                ('cursor', 'Cursor'),
                ('windsurf', 'Windsurf'),
                ('generic', 'Any .mcp.json'),
              ])
                Button(
                  onPressed: () => setState(() => _client = choice.$1),
                  semanticLabel: choice.$2,
                  variant: _client == choice.$1
          ...
          t.gap.y(Space.xl),
          Input(
            key: const ValueKey('library-agent-project'),
            hint: 'Editor project directory',
            semanticLabel: 'Project directory',
            size: InputSize.sm,
          ...
          Text(
            project == null
                ? 'Name the editor project the binding belongs in. '
                    'A project-scoped file lands beside a .lait store, never inside one.'
                : 'Config lands in $project as $name.',
            style: context.monoStyle,
          ),
          t.gap.y(Space.md),
          Row(
            children: [
              Button(
                onPressed: ready
                    ...
                    : null,
                semanticLabel: 'Preview agent binding',
                variant: ButtonVariant.ghost,
              ...
              Button(
                onPressed: ready
                    ...
                    : null,
                semanticLabel: 'Write agent binding',
                variant: ButtonVariant.outline,
                child: Text(writing ? 'Writing' : 'Write binding'),
              ),
            ],
          ),
          if (outcome != null) ...[
            ...
            Text(
              outcome.written
                  ...
                  : 'Would be written to ${outcome.path}',
              style: context.bodyStyle,
            ),
            if (outcome.detail.isNotEmpty) ...[
              t.gap.y(Space.xs),
              Text(outcome.detail, style: context.monoStyle),
            ],
            if (outcome.note != null) ...[
              t.gap.y(Space.xs),
              Text(outcome.note!, style: context.proseStyle),
            ],
          ],
        ],
      ),
...
/// The project directory a project-scoped `.mcp.json` belongs in.
///
/// The Orbit store is `…/.lait`; the binding belongs beside it, not
/// inside it. Any other store path is the Orbit's data, not an editor
/// project.
@visibleForTesting
String? projectDirectory(String? store) {
  ...
  if (cut > 0 && path.substring(cut + 1) == '.lait') {
