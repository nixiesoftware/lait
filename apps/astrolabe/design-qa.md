...
- Copy and content: lifecycle copy distinguishes Ready, Launching, Running, Could not ask, and Unavailable. Running remains a factual state and gains a separate unbordered open-in-new icon; its Go to label remains in semantics and the hover tooltip rather than consuming row width. It never becomes Cancel or Stop. The stable band reports last opened, version, and sync before the far-right settings control. The footer reports local identity, current action, heads, Spaces, and version without exposing a launch credential.
...
- `artifacts/astrolabe-library-running.png` verifies the persistent Running state, separate icon-only Go to action, stable runtime facts, far-right settings control, and successful attachment to an Orbit left running by the existing local identity daemon.
