...
head comparison Exec Watch uses. Approval (`AgentSponsor`) moves the
...
| Trusted runner-local backend | implemented with advisory enforcement | `exec::InProcess` is named relative to the World runner process: it reports `Enforcement::Advisory`, refuses an already-cancelled Context before entry, contains handler panics, and validates the candidate Outcome against the exact selected Spec. The owned runner gives the core host a process lifecycle boundary, but the backend claims no sandbox, container, or kernel resource enforcement inside that runner |
...
| `exec::Cmd` and World command channel | semantic tags 0–7 implemented; World lowering supports Start, Try, Cancel, Accept, and Reject | canonical bytes have golden fixtures and every World effect producer must explicitly stage a command vector; Start pins a Spec and Build while Runtime derives ambient Run coordinates; Try pins a Run, Build, and limits while Runtime writes Station/epoch from the ambient activation and names the Attempt by the identity of the `Leased` event it commits — the signed event's own hash, so a retry advancing from new heads is a distinct Attempt with no monotone counter for two racing Stations to mint alike; Offer and enforcement are optional — a Station-only first Attempt omits both rather than minting a derived OfferId; an Offer, when present, must name this activation; Cancel records only `CancelAsked`; Accept/Reject validate one exact returned Attempt from the callback's pinned snapshot and append the product choice in the same transaction as ordinary World operations; Retry, Resume, and Drain reject until their missing scheduling/service coordinates exist rather than being ignored or guessed |
| Application Work capability | typed runner package implemented for inspect, watch, cancel, continue, and checkpoint resume | `ClientHost::call_work` carries Runtime-owned `WorkRequest` plus a serialized `WorkReply` or typed host refusal, while each application owns the vocabulary and controls that compose it; Issues exposes inspect, watch, cancel, continue, and resume, has no raw Start, and depends on neither the daemon protocol nor Astrolabe; inspect/watch are reads and every mutation enters the same `lower_exec` validator and protected commit path as `World::submit`; continue derives a fresh Attempt — a new `Leased` event, hence a new identity — only from a completed Attempt's committed coordinates in the same Station activation, while resume also requires an exact committed checkpoint and `Resume::Checkpoint`; the current Issues verification Spec is `Resume::Restart`, so it refuses resume with direction to use continue; Started-only, cross-activation, and service-leased work remain typed scheduling refusals rather than guessed transitions; lifecycle projections contain ids, failure class, and returned output ContentRefs, but no product input/output payloads |
...
| Issues verification adopter | semantic Start, runner-local handler, and acceptance transaction implemented; Build publication is identity only | `issue.verify/v1` is declared by IssuesWorld and exposed as `issues_verify`; the application derives the exact Runtime Run from its request coordinate, the World rechecks that binding, and the issue check plus protected `Started` event commit atomically; source is a committed ContentRef; the first-party verifier Build is signed and carried in the selected runner package; callers may omit `build` to select that exact id, and the check then records `package_filled` so a surface cannot present a package default as a caller-named Build; the handler binds the pinned source inside the supervised World runner — it does not compile the repository or provide a sandbox within that runner; the Station drain performs the Attempt locally after Start commits; `issues_accept_check` admits only an exactly-once matching Runtime Outcome and atomically attaches its report, records pass/fail, optionally enters an existing Done state, and stages `Accepted`; no generic `execute-plan` or Work `Start` surface is introduced |
...
| Run, Attempt, and Outcome projections | generation-2 protected Run Body with an intentional generation-1 cutover | Run Body schema 2 and run-event envelope 3 add continuation inputs and durable transcript references as one coordinated change. A generation-1 Run is not re-identified or decoded as generation 2: direct inspection returns typed `UnsupportedRunSchema`, and an old active marker is quarantined from local dispatch so it cannot poison discovery of current Runs or be misreported as store corruption. No lossy DAG migration is attempted. `exec::Run::project` ignores collaborative-list order and rebuilds only Runtime-owned facts; it exposes separate sorted Attempts, Outcomes, cancellation facts, acceptance/rejection facts, and causal heads rather than a scalar LWW status; it refuses malformed DAGs, duplicate Attempt ids, unbound facts, repeated returns, coordinate contradictions, and Run/Attempt limit widening; output bytes remain opaque and only schema, digest, geometry, and ContentRefs enter the generic projection |
| Attempt identity | the `Leased` event's own hash (run-event generation 2) | there is no `Fence` counter and no `derive_attempt_id`: an Attempt is named by `AttemptId::of_event(leased_event_id)`, so its consent, its fencing token, and its idempotency key are one signed fact. Two Stations that race a Run sign different `Leased` events (different station, device, predecessors) and mint different Attempts with nothing to collide on; a retry differs by the heads it advances from. The projection sorts Attempts by that identity, not by insertion order, and the only duplicate Attempt is the identical signed event twice — caught as a duplicate event |
| Liveness facts | `Presumed` and `Renewed` event kinds implemented (tags 10, 11); the deadline-driven producer is not yet wired | a `Presumed` is a third party's **non-terminal** claim an Attempt is dead, carrying `PresumedEvidence` (`Deadline`, `MissedHeartbeat`, `DeadEpoch`) and its presumer — never that it saw the Attempt stop, and never on unreachability alone. It never resolves the Run: the projection keeps the Run unresolved, and a later `Returned`/`Failed` on the same Attempt reconciles over it. A presumption by the executor's own device is a malformed DAG (`self presumption`); a `Renewed` is the executor refreshing its own liveness and must carry the executor device. `FailureClass::Deadline` stays for a performer self-reporting its own blown budget — distinct from a third party's suspicion. The drain that measures a deadline on the reader's monotonic clock and emits these is the next slice |
| Liveness sweep | `Session::sweep_liveness` implemented; heartbeat emission not yet wired | a Station presumes a **foreign** in-flight Attempt dead once `wall_millis + grace` has elapsed on **its own** clock, measured from when this activation first observed the Attempt — never from a timestamp in the record, so no two clocks are compared, and unreachability is never consulted. A renewal it has not seen before resets that anchor. It presumes only an Attempt some other device holds, only once, and only while it has no Outcome/Failure/Cancellation; the `control` demand is enforced at commit. The presumption is non-terminal, so the original executor's later `Returned`/`Failed` reconciles over it. Activation memory: a restart forgets the anchors, which only ever delays a presumption. Proven over two `MemNet` Stations: one begins but never returns, the other presumes it past the deadline and not before, then the executor returns and reconciles |
| Performing another device's Run | consent-gated, per Spec, per Station | a Station always performs Runs its own device started; a Run started by any other device is leased here only when the activation's `exec::Consent` names that exact Spec. Consent is a device's fact set at activation, never a Space's: membership, roles and invites confer no right to run on a device. Refusal is silent — no Try, no lease — because a Run nobody consented to is simply not this Station's work |
| Directed Start | optional target Station on `Start`/`Started` | a Start may name the one Station that must perform it (`Started.target`); only that Station's own activation leases the Run, in addition to the consent every foreign Run needs, and every other Station passes it by whatever its Build or consent. The target is fixed at Start, is not part of the Run id, and travels in the run-event Body — which is why the run-event encoding is generation 2 (`RUN_EVENT_VERSION = 2`); an absent target decodes as open placement exactly as generation 1 did |
| Accept and Reject | owner-written | `Cmd::Accept` and `Cmd::Reject` are lowered only when the ambient principal's device is the device that committed `Started`; any other device is refused with a contract violation before the Outcome is examined. Two Stations may each lease and return the same unresolved Run — both Attempts converge as facts — and the single writer of the closing choice is what keeps "exactly one Accepted" convergent without coordination |
| Local unresolved-Run scan | implemented over an immutable committed Replica generation | `exec::scan_unresolved` selects exact current-generation protected Run bindings for one World, validates the complete event DAG and Body-derived Run identity, reconstructs every exact ordered command chunk, verifies the canonical `Start` digest and duplicated coordinates, and returns Run projections sorted by `RunId`; predecessor-generation active markers are retained as inert history and skipped only after their Run binding is classified as `UnsupportedRunSchema`; malformed current truth and unknown future generations still fail closed. Returned/failed Attempts and Attempt-scoped cancellation remain unresolved, while a Run-level cancellation or any acceptance fact removes the Run from dispatch consideration; the scan is read-only and remote incorporation never invokes it or a handler |
...
bounded deterministic transactions under a launcher-granted capability bound to the
...
| World runner envelope | `world_runner::PROTOCOL_VERSION` | 1 | bounded authenticated loopback Postcard frames |
| World SDK payload | `world_sdk::ABI_VERSION` | 3 | JSON-compatible CBOR semantic, application, client, display, Exec, and callback payloads inside the runner envelope |
...
struct the host fills in, not a constant. Until they get there, this
