...
impl ReachPlane {
    ...
    pub fn restore(
        ...
        let genesis = DeviceLink::seal(&seeds[0], &seeds[1], GENESIS_NONCE, GENESIS_EPOCH)
        ...
        for (index, seed) in seeds.iter().enumerate().skip(2) {
            let link =
                DeviceLink::seal(&seeds[0], seed, GENESIS_NONCE, GENESIS_EPOCH + index as u64)
                    .map_err(|e| ReachError::Kinship(registry::Failure::Kinship(e)))?;
    ...
    pub fn collect(&mut self, carrier: &mut impl Carrier, now: u64) -> Collected {
