...
use crate::head::install_independent_test_worlds;
...
fn the_mcp_head_in_an_empty_dir_refuses_to_create_a_decoy_store() {
    ...
    install_independent_test_worlds(&cfg);
...
fn the_mcp_head_binds_the_sole_registered_orbit_when_the_directory_names_none() {
    ...
    let mut child = Command::new(bin())
        ...
        .current_dir(&cwd)
        .env("LAIT_CONFIG_ROOT", &cfg)
        // The binding authors the World pin; without one, an identity with two
        // installed Worlds refuses before store resolution — not this test.
