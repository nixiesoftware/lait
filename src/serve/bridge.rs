...
//! interest in once per tick, and [`BridgeHub`] fans the answer out. That is
...
const MAX_BRIDGE_BODY_BYTES: usize = MAX_BRIDGE_FRAME_BYTES - 16;
...
impl BridgeHub {
    ...
    pub fn attach_control(&self) -> tokio::sync::mpsc::Receiver<Arc<Vec<u8>>> {
        ...
        let mut sinks = self.control.lock().expect("bridge control sinks");
    ...
    fn refresh(&self, question: &Watch) {
        self.fresh
            .lock()
            .expect("bridge refresh set")
            .insert(question.clone());
    }

    fn take_fresh(&self) -> BTreeSet<Watch> {
        std::mem::take(&mut *self.fresh.lock().expect("bridge refresh set"))
    }

    fn unwatch(&self, question: &Watch) {
        let mut watched = self.watched.lock().expect("bridge watch set");
...
fn encode_body(space: &str, issue: Option<&str>, value: &Response) -> Option<Arc<Vec<u8>>> {
    ...
    if body.len() > MAX_BRIDGE_BODY_BYTES {
        tracing::warn!(
            space,
            bytes = body.len(),
            "a bridge frame past the ceiling was not sent"
        );
        return None;
    }
    Some(Arc::new(body))
...
async fn sweep(app: &App, held: &mut BTreeMap<Watch, u64>) {
    ...
    let fresh = app.bridge.take_fresh();
    let watched = app.bridge.watched();
...
pub(super) async fn session(
    ...
    upgrade
        ...
        // `BridgeFrame::decode` is ever asked about the length, so the check
        // that exists to bound an allocation runs after two of them.
        .max_message_size(MAX_BRIDGE_FRAME_BYTES)
...
async fn run_socket(mut socket: WebSocket, app: &App, watching: &mut Option<Watch>) {
    ...
    let mut transient = app.bridge.subscribe_transient();
    let mut control = app.bridge.attach_control();
...
fn declare(app: &App, watching: &mut Option<Watch>, declared: WatchRequest) {
    ...
    if let Some(previous) = watching.take() {
        app.bridge.unwatch(&previous);
    }
    if let Some(question) = next {
        app.bridge.watch(question.clone());
...
async fn send(socket: &mut WebSocket, lane: Lane, body: &[u8]) -> Result<(), ()> {
    socket
        .send(Message::Binary(
            BridgeFrame::new(lane, body.to_vec()).encode().into(),
        ))
        .await
        .map_err(|_| ())
}
...
mod tests {
    ...
    fn body_of(bytes: &Arc<Vec<u8>>) -> serde_json::Value {
        serde_json::from_slice(bytes).expect("a bridge body is JSON")
    }
    ...
    fn appending_a_lane_left_the_two_before_it_where_they_were() {
        ...
        for (lane, byte) in [
            ...
            let encoded = BridgeFrame::new(lane, Vec::new()).encode();
            assert_eq!(
                encoded[1], byte,
    ...
    fn the_upgrade_bounds_a_message_before_the_decoder_is_asked_about_it() {
        ...
        // and then copied, before `BridgeFrame::decode` is handed a length to
    ...
    fn a_body_past_the_ceiling_is_not_sent_at_all() {
        ...
        let huge = Response::Text {
            text: "x".repeat(MAX_BRIDGE_FRAME_BYTES),
        };
    ...
    fn a_drain_too_large_for_one_frame_is_split_rather_than_lost() {
        ...
        let mut lost = 0u64;
        for body in &bodies {
            assert!(body.len() <= MAX_BRIDGE_BODY_BYTES);
            let value = body_of(body);
