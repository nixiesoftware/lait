...
case "$TARGET" in
  ...
  *)
    ...
    # `tauri build` did stage the reviewed first-party catalog, but this branch
    ...
    # self-update archive consume. It contains metadata and artwork only;
    # executable World payloads arrive through their own signed channels.
    WORLD_CATALOG="$CLIENT/src-tauri/world-catalog"
    [ -d "$WORLD_CATALOG" ] || {
      echo "build-astrolabe: Tauri staged no first-party catalog at $WORLD_CATALOG" >&2
      exit 1
    }
    cp -R "$WORLD_CATALOG" "$STAGE/$LIVE_DIR/world-catalog"
